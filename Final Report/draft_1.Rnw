...
\begin{document}
\SweaveOpts{concordance=TRUE,width=3.5,height=3.5,prefix.string=DA}
...
<<include=FALSE,fig=TRUE,label=ClassBySex,echo=FALSE>>=
...
<<include=FALSE,fig=TRUE,label=ClassByAge,echo=FALSE>>=
bpa <- ggplot(train_csv, aes(factor(Pclass), Age))
bpa + geom_boxplot()
@
<<include=FALSE,fig=TRUE,label=ClassByFare,echo=FALSE>>=
...
\end{figure}
<<include=FALSE,fig=TRUE,label=SurvClass,echo=FALSE>>=
doubledecker(Survived ~ Pclass, data=train_csv)
@

<<include=FALSE,fig=TRUE,label=SurvSex,echo=FALSE>>=
doubledecker(Survived ~ Sex, data=train_csv)
@
<<include=FALSE,fig=TRUE,label=SurvSC,echo=FALSE>>=
